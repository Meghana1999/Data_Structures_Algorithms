...
class Solution:        
    def dfs_helper(self, node):
        ...
        left = self.dfs_helper(node.left)
        right = self.dfs_helper(node.right)
        ...
        cursum = max(left, right) + node.data
        allsum = left + right + node.data
         
        self.maxsum = max(self.maxsum, allsum)
        return cursum
